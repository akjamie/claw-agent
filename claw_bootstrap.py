"""Windows UTF-8 bootstrap for Claw entry points.
...
should be imported at the very top of every Claw entry point
(``Claw``, ``Claw-agent``, ``Claw-acp``, ``python -m gateway.run``,
...
# Apply on# Apply on import — entry points just need ``import claw_bootstrap``
# (or ``from claw_bootstrap import apply_windows_utf8_bootstrap``) atp of their module, before importing anything else.  The
